...
<script lang="ts">
	...
	function submitGuess() {
		...
		const clueIndex = clues.findIndex((c) => !c.solved && c.answer.toUpperCase() === guess);
		if (clueIndex !== -1) {
			// Correct guess
...
<div class="game-container">
			...
			<div class="clue-item">
				...
				<div class="clue-answer">
					...
					{:else}
						...
						{Array.from({ length: clue.answer.length })
							.map(() => '_')
							.join(' ')}
					{/if}
				</div>
	...
	<div class="selected">
		...
		<div class="actions">
			<button on:click={submitGuess} disabled={selectedGroups.length === 0}>Submit</button>
			<button on:click={undo} disabled={selectedGroups.length === 0}>Undo</button>
			<button on:click={clearSelection} disabled={selectedGroups.length === 0}>Clear</button>
		</div>
		{#if message}
