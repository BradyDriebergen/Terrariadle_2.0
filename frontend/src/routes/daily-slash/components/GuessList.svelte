...
<div in:fly={{ x: 800, duration: 1000 }} >
    <div class="header">
        ...
        <span title="Snail, Extremely Slow, Very Slow, Slow, Average, Fast, Very Fast, Insanely Fast">Use Time</span>
        <span title="White, Blue, Green, Orange, Light_Red, Pink, Light_Purple, Lime, Yellow, Cyan, Red">Rarity</span>
    ...
    {#each guesses as guess, i (guess.id)}
        <div class="row" animate:flip>
            ...
            <span style={checks[i].DamageType ? guessCorrect : guessWrong} in:fly={{ x: 700, duration: 2000 }}>
            ...
            <span
                ...
                style={((checks[i].Rarity === 1) ? guessCorrect : guessWrong) + getRarityColor(guess.info.rarity as string)}
                in:fly={{ x: 400, duration: 2000 }}
            >
                {guess.info.rarity}
            </span>

            <span style={checks[i].Operation ? guessCorrect : guessWrong} in:fly={{ x: 300, duration: 2000 }}>
                {guess.info.operation}
            </span>
            <span style={checks[i].Material ? guessCorrect : guessWrong} in:fly={{ x: 200, duration: 2000 }}>
                {guess.info.material}
            </span>

            <div style="flex-direction: column; {checkedObtained(checks[i].Obtained)}" in:fly={{ x: 100, duration: 2000 }}>
